/* main.cc (port) — adapted from tts_zh/tts_src/main.cc for big-core Linux.
 *
 * Changes vs vendor:
 *  - k230_kpu_init() (gnne_regs preseed + gnne_init) runs before the
 *    three AIBase constructors (kmodel load needs KPU state up).
 *  - stdin loop runs until EOF instead of the vendor's fixed 3 turns;
 *    an optional argv[5] text argument synthesizes once and exits
 *    (the LLM wiring path: one process per utterance).
 *  - output path per iteration: $TTS_OUT if set (overwritten each turn),
 *    else ./tts_out_<n>.wav.
 *  - per-turn wall-clock timing printed to stderr.
 */
#include <iostream>
#include <thread>
#include <cmath>
...
{
    cout << "Usage: " << name << "<kmodel_fastspeech1> <kmodel_fastspeech2> <kmodel_hifigan> <debug_mode> [text]" << endl
         << "Options:" << endl
         ...
         << "  text            可选：直接合成该文本后退出；不填则从 stdin 逐行合成\n"
         << "\n"
         << endl;
}
...
/* NOTE: cleanup() never actually runs — main() finishes with _exit(0)
 * (see below). The nncase k230 runtime's static destructors dereference
 * mmz segments; freeing them in any teardown order segfaults at exit
 * (same class as llama's historical rc=134). We release the segments
 * explicitly and skip all static teardown instead. */

/* one full utterance through the vendor pipeline; returns wall-clock ms */
static double synth_once(Fastspeech1 &fspeech1, Fastspeech2 &fspeech2,
                         HiFiGan &hifigan, zh_frontend &front,
                         map<string, int> &symbol_to_id,
                         const string &text, const string &wav_path)
{
    ...
    string text_norm = std::regex_replace(text, RE_DOUHAO_NUM, "$1$3");
    //文本转拼音
    vector<vector<string>> pinyin = front.get_phonemes(text_norm,false,true,false,false);
    //拼音转音素
    ...
    std::vector<int16_t> int16_results;

    // 将mel频谱数据float数据转换为int16_t类型的PCM数据
    for (float num : results_mel_all) {
        uint8_t bytes[2];
        num = std::max(std::min(num, 1.f), -1.f);
        int16_t converted_num = static_cast<int16_t>(num* 32767.);
        int16_results.push_back(converted_num);
    }
    unsigned int data_len=int16_results.size()*sizeof(int16_t);

    // $TTS_OUT set -> fixed path (LLM wiring, one utterance per run);
    // unset -> numbered file per stdin turn (batch testing).
    const char *env = getenv("TTS_OUT");
    if (!env || !env[0]) setenv("TTS_OUT", wav_path.c_str(), 1);

    // 获取int16_t数据的指针并转换为char*
    char* char_ptr = reinterpret_cast<char*>(int16_results.data());
    playerPcm(1,sample_rate,char_ptr,data_len);
...
{  
    ...
    // KPU scaffolding must run BEFORE the kmodels load (AIBase ctors)
    k230_kpu_init();
    ...
    int turn = 0;
    auto synth = [&](const string &text) {
        char path_buf[128];
        snprintf(path_buf, sizeof(path_buf), "tts_out_%d.wav", turn);
        string wav_path = path_buf;
        double ms = synth_once(fspeech1, fspeech2, hifigan, zh_frontend,
                               symbol_to_id, text, wav_path);
        fprintf(stderr, "[tts] turn %d: %zu chars -> %s (%.0f ms)\n",
                turn, text.size(), wav_path.c_str(), ms);
        turn++;
    };
