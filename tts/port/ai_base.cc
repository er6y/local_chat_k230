/* ai_base.cc (port) — adapted from common/ai_base.cc for nncase 2.11.
 *
 * Changes vs vendor:
 *  - dropped #include "utils.h" (pulls opencv; nothing here uses it)
 *  - dropped `using namespace nncase::runtime::detail;` (2.11 keeps the
 *    hrt alias in nncase::runtime proper)
 *  - get_output() rewritten for the 2.11 API: the old
 *    out.impl()->to_host()->buffer().as_host()->map() chain is gone, and
 *    to_host() now returns a NEW host tensor whose storage dies with it,
 *    so the tensors + map views are kept alive in class members while
 *    p_outputs_ pointers are read by the post_process() callers.
 */
#include "ai_base.h"
...
void AIBase::set_input_init()
{
    ...
    for (int i = 0; i < kmodel_interp_.inputs_size(); ++i)
    {
        ...
        vector<int> in_shape;
        if (debug_mode_ > 1)
            cout<<"input "<< std::to_string(i) <<" : "<< (int)desc.datatype << ",";
        int dsize = 1;
        ...
        {
            printf("input data type:%d",(int)desc.datatype);
...
void AIBase::set_output_init()
{
    ...
    for (size_t i = 0; i < kmodel_interp_.outputs_size(); i++)
    {
        ...
        if (debug_mode_ > 1)
            cout<<"output "<<std::to_string(i)<<" : "<< (int)desc.datatype << ",";
        int dsize = 1;
        for (int j = 0; j < shape.size(); ++j)
        {
        ...
        {
            printf("output data type:%d",(int)desc.datatype);
            assert(("unsupported kmodel output data type", 0));
        }
...
void AIBase::get_output()
{
    ...
    for (int i = 0; i < kmodel_interp_.outputs_size(); i++)
    {
        ...
        auto th = out.to_host().expect("cannot to_host output tensor");
        auto omr = host_runtime_tensor::map(th, map_read);
        if (!omr.is_ok())
        {
            printf("map output %d failed\n", i);
            continue;
        }
        host_outs_.push_back(th);
        mapped_outs_.push_back(std::move(omr.unwrap()));
        float *p_out = reinterpret_cast<float *>(mapped_outs_.back().buffer().data());
