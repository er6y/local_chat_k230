...
PC = '/tmp/matcha-icefall-zh-baker/matcha_C128m.onnx'
m = onnx.load(PC)
...
if try_compile('/tmp/pref_m.onnx') if build_prefix(50, '/tmp/pref_m.onnx') is None else False:
    pass
build_prefix(50, '/tmp/pref_m.onnx')
if try_compile('/tmp/pref_m.onnx'):
    print('prefix 50 OK')
else:
...
while hi - lo > 1:
    mid = (lo + hi) // 2
    build_prefix(mid, '/tmp/pref_m.onnx')
    ok = try_compile('/tmp/pref_m.onnx')
...
build_prefix(hi, '/tmp/pref_m_crash.onnx')
mm = onnx.load('/tmp/pref_m_crash.onnx')
bad = mm.graph.node[-1]
