...
[RuleGenerator]
public sealed class MarkActivationInputBL1Fuse : RewriteRule<Pattern>, IRewriteRule
{
	public override Pattern Pattern { get; } = Nncase.PatternMatch.Utility.IsCallWildcard("actCall", Nncase.PatternMatch.Utility.IsOp<GNNEActivation>("act"), Nncase.PatternMatch.Utility.IsWildcard("inputA"), Nncase.PatternMatch.F.K230.IsGNNELoad("ld", "ldCall", (GNNELoad _) => true, Nncase.PatternMatch.Utility.IsCallWildcard("quantCall", Nncase.PatternMatch.Utility.IsOp<Quantize>(), Nncase.PatternMatch.Utility.IsCallWildcard("stCall", Nncase.PatternMatch.Utility.IsOp<GNNEStore>(), Nncase.PatternMatch.Utility.IsCallWildcard("convCall", Nncase.PatternMatch.Utility.IsOp<GNNEConv2D>(), Nncase.PatternMatch.Utility.IsWildcard("input"))))));
	...
	private Expr? GetReplace(GNNEActivation act, Call stCall, Call ldCall, Call actCall, Call convCall, IReadOnlyList<Expr> actCallParams, RunPassContext context, Call quantCall)
	{
		...
		if (analysis[stCall].Count() > 1 || analysis[quantCall].Count() > 1 || analysis[ldCall].Count() > 1 || act.InputFromL1[0] || actCall.CheckedShape != convCall.CheckedShape)
		{
			return null;
		}
		Call item = Nncase.IR.K230.F.Tensors.GNNELoad(DataTypes.Float16, stCall);
		IRArray<bool>? inputFromL = new bool[2] { false, true };
