...
[RuleGenerator]
public class ACosToFakeActivation : UnaryToFakeActivation
{
	...
	private Expr? GetReplace(Call call, Expr input)
	{
		...
		ActFun actFun = new ActFun
		{
			...
			MaxParam = new List<float> { 0f, 0f }
		};
		actFun.Func = (float x) => MathF.Acos(x);
		Set_seg_fitting_param(actParam, actFun);
		Tensor<float> tensor = new Tensor<float>(actParam.GetAct1Data, new int[]{1,49});
		return Nncase.IR.K230.F.Tensors.FakeActivation(input, None.Default, tensor, 1, 0, 0, 0, true, GnneActivationType.Add, actParam, call.CheckedShape.ToValueArray());
	}
	...
	public ACosToFakeActivation()
	{
		Func<Unary, bool> condition = (Unary b) => b.UnaryOp == UnaryOp.Acos;
