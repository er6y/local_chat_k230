...
c.import_onnx(open('/ux/work/yilei.wang/k230/qwen25_24l_s1h256_kv6_stacked.onnx', 'rb').read(), nncase.ImportOptions())
print('import %.1fs' % (time.time() - t0), flush=True)
c.use_ptq(ptq)
c.compile(); print('compile %.1fs' % (time.time() - t0), flush=True)
code = c.gencode_tobytes()
open('/ux/work/yilei.wang/k230/llm_kv6_stacked.kmodel', 'wb').write(code)
