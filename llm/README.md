...
| 官方模型真实成绩（官方运行时+正确 KV 契约，逐步实测） | 243ms 平坦（H=13→63） | 同板同日同 poke；我们 python 旧测法 290-310ms 系错误 KV 形状所致（已勘误） |
| 官方公开标尺 | ~202ms（4.9 tok/s） | 至今未复现（今日最优 243ms）；另官方演示程序长生成 ~53 步必崩（CMA 耗尽），我们的方案零泄漏 |
| 目标 | ≤250ms | 差 18ms |
