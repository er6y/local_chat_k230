...
[EvaluatorGenerator]
[TypeInferGenerator]
public class GNNELoadWEvaluator : IEvaluator<GNNELoadW>, IEvaluator, ITypeInferencer<GNNELoadW>, ITypeInferencer, ICostEvaluator<GNNELoadW>, ICostEvaluator
{
	...
	public IRType Visit(GNNELoadW target, TensorType input)
	{
		...
		(DataType, PrimType) tuple2 = tuple;
		if (tuple2.Item1 != tuple2.Item2 && (!(tuple2.Item1 == DataTypes.UInt8) || !(tuple2.Item2 == DataTypes.Int8)) && (!(tuple2.Item1 == DataTypes.Int8) || !(tuple2.Item2 == DataTypes.UInt8)) && (!(tuple2.Item1 == DataTypes.Float32) || !(tuple2.Item2 == DataTypes.Float16)))
		{
			return new InvalidType("load input type and output type should be same");
		}
		(DataType, PrimType) tuple3 = tuple;
		if (tuple3.Item2 != DataTypes.Int8 && tuple3.Item2 != DataTypes.Int16 && tuple3.Item2 != DataTypes.Float16 && tuple3.Item2 != DataTypes.Float32 && tuple3.Item2 != DataTypes.UInt8 && tuple3.Item2 != ExtDataTypes.DeQuantParam)
		{
			return new InvalidType("load output type should be one of [int8, int16, float16, uint8]");
		}
		(DataType, PrimType) tuple4 = tuple;
		if (tuple4.Item1 == DataTypes.Int8 || tuple4.Item1 == DataTypes.Int16 || tuple4.Item1 == DataTypes.Float16 || tuple4.Item1 == DataTypes.Float32 || tuple4.Item1 == DataTypes.UInt8 || tuple4.Item1 == ExtDataTypes.DeQuantParam)
		{
			TensorType tensorType = new TensorType(tuple4.Item1, input.Shape);
			return tensorType;
		}
