...
[RuleGenerator]
public sealed class ReduceWithDim12ToGlobalReduceWindow : IRewriteRule
{
	...
	private Expr? GetReplace(Reduce reduce, Call call, Expr input, TensorConst axis, TensorConst initValue, Marker inputMarker, Marker outputMarker, Expr inputRange, Expr outputRange)
	{
		...
		Marker input2 = Nncase.IR.F.Math.RangeOfMarker(Nncase.IR.F.Tensors.Transpose(inputMarker, new int[4] { 0, 3, 1, 2 }), inputRange).With(null, null, null, adaQuantInfo: inputMarker.AdaQuantInfo, mixQuantInfo: inputMarker.MixQuantInfo);
		return Nncase.IR.F.Math.RangeOfMarker(Nncase.IR.F.Tensors.Transpose(Nncase.IR.F.Math.RangeOfMarker(Nncase.IR.F.NN.ReduceWindow2D(reduce.ReduceOp, input2, initValue, new int[2] { 3, 3 }, new int[2] { 2, 1 }, new int[2, 2]
		{
			{ 1, 1 },
			{ 1, 1 }
		}, new int[2] { 1, 1 }, false, false), outputRange).With(null, null, null, adaQuantInfo: outputMarker.AdaQuantInfo, mixQuantInfo: outputMarker.MixQuantInfo), new int[4] { 0, 2, 3, 1 }), outputRange).With(null, null, null, adaQuantInfo: outputMarker.AdaQuantInfo, mixQuantInfo: outputMarker.MixQuantInfo);
	}

	private bool TryMatch(int[] outShape, int[] inShape, int[] axisValue)
	{
		if (outShape.Length == 4 && inShape.Length == 4 && axisValue.Length == 2 && axisValue[0] == 1 && axisValue[1] == 2)
		{
			return true;
		}
		return false;
