...
[RuleGenerator]
public class TileTransposeMatmul : RewriteRule<Pattern>
{
	...
	private PrimFunction GetReplace(Call call, Call ld, Call st, Fusion fusion)
	{
		...
		TileTransposeMatmulGlb glb = SearchGlbParameters();
		...
		ScheduleStrategy index = ScheduleStrategy.IfFirstSchedule;
		List<GnneAction> actions = list2;
		Span<byte> bytesBuffer = ((TensorConst)((Call)buffer3.MemSpan.Start)[DDrOf.Input]).Value.BytesBuffer;
		ArrangeWeights(_weightType, _weightsShape, bytesBuffer, _weightGroups[(int)index]);
		ActionToInstruct actionToInstruct = new ActionToInstruct();
		return T.PrimFunc($"TileTransposeMatmul_{_count}", K230RtModule.Kind, list.ToArray()).Body(actionToInstruct.Instructions(actions), I.END(GP_REGISTER.x0, 0)).Build();
	}
	...
	private List<GnneAction> BuildIfFirstSchedule(TiledGlb glb, Call convCall, Call ld, Call st, Nncase.TIR.Buffer ddrIf, Nncase.TIR.Buffer ddrOf, Nncase.TIR.Buffer ddrW, Nncase.TIR.Buffer ddrWQarg, Nncase.TIR.Buffer ddrAct, Nncase.TIR.Buffer ddrDW, Nncase.TIR.Buffer ddrDWQarg, Nncase.TIR.Buffer ddrDWAct, Nncase.TIR.Buffer ddrAct1, Nncase.TIR.Buffer ddrPdpAct, Nncase.TIR.Buffer ddrIf2, bool weightGroupOnly, WeightGroupHandler weightGroup)
	{
		...
		if (glb.LastOutShape[1] >= _ocPerGroup)
		{
			...
			if (_tp1)
			{
				list3 = TileUtilities.GetSegmentStartEndLength(0, glb.GlbMap[ItemName.Ifmap2].Dimensions[3], _inputShape[1]);
			}
		}
		else
		{
			for (int i = 0; i < _groups; i++)
			{
				...
				if (_tp1)
				{
					segmentStartEndLength5 = TileUtilities.GetSegmentStartEndLength(i * _icPerGroup, glb.GlbMap[ItemName.Ifmap2].Dimensions[3], (i + 1) * _icPerGroup);
				}
				...
				foreach (Segment1D item4 in segmentStartEndLength3)
				{
					...
					foreach (Segment1D item5 in list2)
					{
						...
						foreach (Segment1D item6 in list3)
						{
							...
							SegmentND segmentND4 = segmentND3;
							if (_tp1)
							{
								segmentND4 = new SegmentND(item2, inputRowSegment2, inputColumnSegment2, item6);
								SegmentND ifmap = new SegmentND(inputColumnSegment2, item6, item2, inputRowSegment2);
								if (list5[0] == segmentND4)
								{
									num2 = 0;
								}
								else if (list5[1] == segmentND4)
								{
									num2 = 1;
								}
								else
								{
									num2 = (num2 + 1) % 2;
									if (!weightGroupOnly && _g2LIfRec[num2].Count > 0 && _g2LIfRec[num2][0].Item1 == segmentND4)
									{
										...
										{
											new CcrSet(_ccrHandler.GetCcrItem(_ccrHandler.GetName(ItemName.Ifmap2, num2)), value)
										};
										...
										GNNEShape shape = new GNNEShape(segmentND4[0].Length, segmentND4[1].Length, segmentND4[2].Length, segmentND4[3].Length);
										gnneActionUpdater.UpdateLoadIf(ifmap, _lif, num2, ddrIf, 0, stridesD, ItemName.Ifmap2, ccrsToSet, null, _h2C, ((TensorConst)convCall[GNNEConv2D.DeqBias]).Value.ToScalar<int>(), stridesS, shape);
									}
									list5[num2] = segmentND4;
								}
							}
							else if (list5[0] == segmentND3)
							{
								num2 = 0;
							}
							...
							else
							{
								...
								if (!weightGroupOnly && _g2LIfRec[num2].Count > 0 && _g2LIfRec[num2][0].Item1 == segmentND3)
								{
									...
									gnneActionUpdater.UpdateLoadIf(segmentND3, _lif, num2, ddrIf, 0, stridesD2, ItemName.Ifmap, ccrsToSet2, null, _h2C, ((TensorConst)convCall[GNNEConv2D.DeqBias]).Value.ToScalar<int>());
								}
								list5[num2] = segmentND3;
							}
							if (!weightGroupOnly && (object)_act1 != null && _act1[GNNEActivation.InputB] != None.Default && item6.End == _inputShape[1])
							{
								segmentND4 = segmentND;
								TensorStat item = _l2RIf2Rec[num][0].Item2;
								int value3 = ((item != null && item.IsFirstSlice && item.IsLastSlice) ? 1 : 2);
								List<CcrSet> ccrsToSet3 = new List<CcrSet>
								{
									new CcrSet(_ccrHandler.GetCcrItem(_ccrHandler.GetName(ItemName.Ifmap2, num)), value3)
								};
								List<int> stridesD3 = new List<int>
								{
									glb.GlbMap[ItemName.Ifmap2].Dimensions[1],
									glb.GlbMap[ItemName.Ifmap2].Dimensions[2],
									glb.GlbMap[ItemName.Ifmap2].Dimensions[3]
								};
								gnneActionUpdater.UpdateLoadIf(segmentND4, _lif2, num, ddrIf2, 0, stridesD3, ItemName.Ifmap2, ccrsToSet3);
							}
							BuildL1Schedule(gnneActionUpdater, glb, segmentND3, segmentND2, segmentND, num2, num, num3, segmentND4, weightGroupOnly, weightGroup, l1Pp, ddrW, ddrOf);
						}
					}
				}
			}
		}
		TileUtilities.Assert(_ccrHandler.CcrSanityCheck(), "_ccrHandler.CcrSanityCheck()", "/home/gitlab-runner/builds/zaC7hZ1H/0/maix2-ai-sw/k510-gnne-compiler/modules/Nncase.Modules.K230/Transform/Rules/Tile/TileTransposeMatmul.cs", 466);
		return list;
	}
	...
	private List<GnneAction> BuildWFirstSchedule(TiledGlb glb, Call convCall, Call ld, Call st, Nncase.TIR.Buffer ddrIf, Nncase.TIR.Buffer ddrOf, Nncase.TIR.Buffer ddrW, Nncase.TIR.Buffer ddrWQarg, Nncase.TIR.Buffer ddrAct, Nncase.TIR.Buffer ddrDW, Nncase.TIR.Buffer ddrDWQarg, Nncase.TIR.Buffer ddrDWAct, Nncase.TIR.Buffer ddrAct1, Nncase.TIR.Buffer ddrPdpAct, Nncase.TIR.Buffer ddrIf2, bool weightGroupOnly, WeightGroupHandler weightGroup)
	{
				...
				foreach (Segment1D item4 in segmentStartEndLength2)
				{
					...
					foreach (Segment1D item5 in segmentStartEndLength3)
					{
						...
						foreach (Segment1D item6 in list3)
						{
							...
							if (!weightGroupOnly && (object)_act1 != null)
							{
								Call call = (Call)_act1[GNNEActivation.InputB];
								if (((object)call == null || !(call.Target is Uninitialized)) && item6.End == _inputShape[1])
								{
									TensorStat item = _l2RIf2Rec[num][0].Item2;
									int value2 = ((item != null && item.IsFirstSlice && item.IsLastSlice) ? 1 : 2);
									List<CcrSet> ccrsToSet2 = new List<CcrSet>
		...
		TileUtilities.Assert(_ccrHandler.CcrSanityCheck(), "_ccrHandler.CcrSanityCheck()", "/home/gitlab-runner/builds/zaC7hZ1H/0/maix2-ai-sw/k510-gnne-compiler/modules/Nncase.Modules.K230/Transform/Rules/Tile/TileTransposeMatmul.cs", 703);
		return list;
	}

	private TileTransposeMatmulGlb SearchGlbParameters()
	{
		int i = 1;
		int j = 1;
		...
		int inputHeight = SpaceSearcher.GetInputHeight(num3, _convOutputShape[2], _fusedKernelH, _outputShape[2], _fusedStrideH, _fusedDilationH, in _fusedPaddingH);
		...
		int num5 = SpaceSearcher.GetInputHeight(inputHeight, inH, num, outH, strideH, dilationH, in p);
		int inputHeight2 = SpaceSearcher.GetInputHeight(num4, _convOutputShape[3], _fusedKernelW, _outputShape[3], _fusedStrideW, _fusedDilationW, in _fusedPaddingW);
		int inH2 = _inputShape[3] + _paddingW.Sum();
		int outH2 = _convOutputShape[3];
		int strideW = _strideW;
		int dilationW = _dilationW;
		p = new Padding(0, 0);
		int num6 = SpaceSearcher.GetInputHeight(inputHeight2, inH2, num2, outH2, strideW, dilationW, in p);
		int k = 1;
		...
		if (_strategy == GlbSearchStrategy.IfFirst)
		{
			...
			{
				int num11 = num3 + 1;
				int inputHeight3 = SpaceSearcher.GetInputHeight(num11, _convOutputShape[2], _fusedKernelH, _outputShape[2], _fusedStrideH, _fusedDilationH, in _fusedPaddingH);
				int inH3 = _inputShape[2] + _paddingH.Sum();
				int outH3 = _convOutputShape[2];
				int strideH2 = _strideH;
				int dilationH2 = _dilationH;
				p = new Padding(0, 0);
				int inputHeight4 = SpaceSearcher.GetInputHeight(inputHeight3, inH3, num, outH3, strideH2, dilationH2, in p);
				allocateResult = HandleAllocate(i, j, inputHeight4, num6, num, num2, k, num11, num4);
				if (!allocateResult.IsOk)
				{
					break;
				}
			...
			{
				int num12 = num4 + 1;
				int inputHeight5 = SpaceSearcher.GetInputHeight(num12, _convOutputShape[3], _fusedKernelW, _outputShape[3], _fusedStrideW, _fusedDilationW, in _fusedPaddingW);
				int inH4 = _inputShape[3] + _paddingW.Sum();
				int outH4 = _convOutputShape[3];
				int strideW2 = _strideW;
				int dilationW2 = _dilationW;
				p = new Padding(0, 0);
				int inputHeight6 = SpaceSearcher.GetInputHeight(inputHeight5, inH4, num2, outH4, strideW2, dilationW2, in p);
				allocateResult = HandleAllocate(i, j, num5, inputHeight6, num, num2, k, num3, num12);
				if (!allocateResult.IsOk)
				{
					break;
				}
		...
		else if (_strategy == GlbSearchStrategy.WFirst)
		{
			...
			{
				int num13 = num3 + 1;
				int inputHeight7 = SpaceSearcher.GetInputHeight(num13, _convOutputShape[2], _fusedKernelH, _outputShape[2], _fusedStrideH, _fusedDilationH, in _fusedPaddingH);
				int inH5 = _inputShape[2] + _paddingH.Sum();
				int outH5 = _convOutputShape[2];
				int strideH3 = _strideH;
				int dilationH3 = _dilationH;
				p = new Padding(0, 0);
				int inputHeight8 = SpaceSearcher.GetInputHeight(inputHeight7, inH5, num, outH5, strideH3, dilationH3, in p);
				allocateResult = HandleAllocate(i, j, inputHeight8, num6, num, num2, k, num13, num4);
				if (!allocateResult.IsOk)
				{
					break;
				}
			...
			{
				int num14 = num4 + 1;
				int inputHeight9 = SpaceSearcher.GetInputHeight(num14, _convOutputShape[3], _fusedKernelW, _outputShape[3], _fusedStrideW, _fusedDilationW, in _fusedPaddingW);
				int inH6 = _inputShape[3] + _paddingW.Sum();
				int outH6 = _convOutputShape[3];
				int strideW3 = _strideW;
				int dilationW3 = _dilationW;
				p = new Padding(0, 0);
				int inputHeight10 = SpaceSearcher.GetInputHeight(inputHeight9, inH6, num2, outH6, strideW3, dilationW3, in p);
				allocateResult = HandleAllocate(i, j, num5, inputHeight10, num, num2, k, num3, num14);
				if (!allocateResult.IsOk)
				{
					break;
				}
		...
		TileUtilities.Assert(allocateResult.IsOk, "allocation.IsOk", "/home/gitlab-runner/builds/zaC7hZ1H/0/maix2-ai-sw/k510-gnne-compiler/modules/Nncase.Modules.K230/Transform/Rules/Tile/TileTransposeMatmul.cs", 970);
		GNNEShape gNNEShape = new GNNEShape(i, k, num3, num4);
		System.IO.File.AppendAllText("/tmp/k230_srv/tiledbg.log", "TILEDBG Tmm TransposeMatmul in=[" + string.Join(",", _inputShape) + "] out=[" + string.Join(",", _outputShape) + "] w=[" + string.Join(",", _weightsShape) + "] glbTile=(n=" + i + ",c=" + k + ",h=" + num3 + ",w=" + num4 + ") j=" + j + " strategy=" + _strategy);
		return new TileTransposeMatmulGlb(allocateResult.GlbMap, allocateResult.Items, gNNEShape.Dims, GNNEEnv.NPingPongSplit, allocateResult.GlbMap[ItemName.Ifmap], allocateResult.GlbMap[ItemName.Ofmap]);
	}
	...
	private List<int> L1Search(TiledGlb glb, SegmentND weight, SegmentND psum)
	{
		...
		while (f < psum[3].Length && h * w * ifBytesPerElementGlb < GNNEEnv.IfL1SizePerChan / 2 && IncreaseFBy1())
		{
		}
		while (e < psum[2].Length && h * w * ifBytesPerElementGlb < GNNEEnv.IfL1SizePerChan / 2 && IncreaseEBy1())
		{
		}
		if (_weightSplitPattern.Equals(new Tuple<int, int>(0, 0)))
		{
			...
			while (s < weight[3].Length && s < 8 && _dilationW <= 3 && IncreaseSBy1())
			{
			}
			while (r < weight[2].Length && r < 31 && _dilationH <= 3 && IncreaseRBy1())
			{
			}
		}
		...
		while (f < psum[3].Length && IncreaseFBy1())
		{
		}
		while (e < psum[2].Length && IncreaseEBy1())
		{
		}
		if (_dilationW > 3)
		{
	...
	private void BuildL1Schedule(GnneActionUpdater actionUpdater, TiledGlb glb, SegmentND ifmap, SegmentND weight, SegmentND psum, int iPp, int ofPp, int wPp, SegmentND ifmap2, bool weightGroupOnly, WeightGroupHandler weightGroup, int l1Pp, Nncase.TIR.Buffer ddrW, Nncase.TIR.Buffer ddrOf)
	{
		List<int> list = L1Search(glb, weight, psum);
		System.IO.File.AppendAllText("/tmp/k230_srv/tiledbg.log", "TILEDBG Tmm L1Search=[" + string.Join(",", list) + "]"); List<Segment1D> segmentStartEndLength = TileUtilities.GetSegmentStartEndLength(psum[2].Start, list[2], psum[2].End);
				...
				foreach (Segment1D item3 in segmentStartEndLength)
				{
					...
					foreach (Segment1D item4 in segmentStartEndLength2)
					{
						...
						foreach (Segment1D item5 in list3)
						{
							...
							segment1D = new Segment1D(new System.Range(end: Math.Min(num4 + item5.Length, _icPerGroup), start: num4), new Padding(0, 0));
												...
												for (int num7 = 0; num7 < num6; num7++)
												{
													if (!weightGroupOnly)
													{
														...
														if (!_weightPreloadEn)
														{
															...
															TileUtilities.Assert(w2 == tuple3.Item1 && tuple2.Item1 == tuple.Item1, "l2RW == g2RWSliceRecStat.Item1 && g2RWRecStat.Item1 == weightRecStat.Item1", "/home/gitlab-runner/builds/zaC7hZ1H/0/maix2-ai-sw/k510-gnne-compiler/modules/Nncase.Modules.K230/Transform/Rules/Tile/TileTransposeMatmul.cs", 1376);
															if (tuple3.Item2.IsFirstSlice)
															{
																...
																if (num8 > 0)
																{
																	list4.Add(new CcrClr(_ccrHandler.GetCcrItem(_ccrHandler.GetName(ItemName.WeightFake, wPp))));
																}
																List<CcrSet> ccrsToSet = new List<CcrSet>
																{
																	new CcrSet(_ccrHandler.GetCcrItem(_ccrHandler.GetName(ItemName.Weight, (wPp << 1) + (tuple3.Item2.SliceIdx & 1))), 1)
																};
																actionUpdater.UpdateLoadW(w2, _lw, weightGroup, wPp, ddrW, ccrsToSet, list4, 0, _h2C, 0, ItemName.Weight, i);
															}
															int num9 = 0;
															...
															if (num9 > 0)
															{
																list5.Add(new CcrSet(_ccrHandler.GetCcrItem(_ccrHandler.GetName(ItemName.WeightFake, wPp)), 1));
															}
															List<CcrClr> list6 = new List<CcrClr>();
															if (tuple3.Item2.IsFirstSlice)
															{
																list6.Add(new CcrClr(_ccrHandler.GetCcrItem(_ccrHandler.GetName(ItemName.Weight, (wPp << 1) + (tuple3.Item2.SliceIdx & 1)))));
															}
															actionUpdater.UpdateG2RW(w2, weightGroup, _ocPerGroup, _lw, wPp, i, list5, list6, 0, 0, ItemName.Weight, ItemName.WQarg, _h2C);
														}
														else
														{
													...
													if (segmentND2[0].Length > 0 && segmentND2[1].Length > 0 && segmentND2[2].Length > 0 && segmentND2[3].Length > 0)
													{
														...
														if (_tp1)
														{
															if (num7 != 1 && i != 1)
															{
																ifmap2 = new SegmentND(ifmap[0], ifmap[2], ifmap[3], ifmap[1]);
																if (!weightGroupOnly)
																{
																	int num10 = _g2LIfRec[iPp][0].Item2.Stat_cnt();
																	_g2LIfRec[iPp].RemoveAt(0);
																	List<CcrClr> list7 = new List<CcrClr>();
																	if (num10 > 0)
																	{
																		list7.Add(new CcrClr(_ccrHandler.GetCcrItem(_ccrHandler.GetName(ItemName.Ifmap2, iPp))));
																	}
																	SegmentND segmentND3 = new SegmentND(segmentND2[0], segmentND2[2], segmentND2[3], segmentND2[1]);
																	List<CcrSet> list8 = new List<CcrSet>();
																	list8.Add(new CcrSet(_ccrHandler.GetCcrItem(_ccrHandler.GetName(ItemName.Ifmap, num)), 1));
																	int dim = (segmentND3[0].Start - ifmap2[0].Start) % ifmap2[0].Length;
																	int dim2 = (segmentND3[1].Start - ifmap2[1].Start) % ifmap2[1].Length;
																	int dim3 = (segmentND3[2].Start - ifmap2[2].Start) % ifmap2[2].Length;
																	int dim4 = (segmentND3[3].Start - ifmap2[3].Start) % ifmap2[3].Length;
																	int addr = glb.GlbMap[ItemName.Ifmap2].GetAddr(dim, dim2, dim3, dim4);
																	int offsetD = 0;
																	actionUpdater.UpdateMfuTranspose(segmentND3, segmentND2, _inputType, MFU_TRANS_PERMUTE.NWCH, iPp, list8, list7, addr, offsetD, ItemName.Ifmap2, ItemName.Ifmap, num);
																	list7 = new List<CcrClr>();
																	list7.Add(new CcrClr(_ccrHandler.GetCcrItem(_ccrHandler.GetName(ItemName.Ifmap, num))));
																	actionUpdater.UpdateG2LIf(segmentND2, ifmap, _lif, num, null, list7, 0, DataTypes.UInt8, ItemName.Ifmap, restored: false, 0, 0, 0, _h2C, _weightsShape[2], _strideH, slice_from: false);
																}
																else
																{
																	_g2LIfRec[iPp].Add(new Tuple<SegmentND, TensorStat>(ifmap2, new TensorStat(isFirstSlice: false, isLastSlice: false)));
																}
															}
															if (num6 == 2)
															{
														...
														else if (!weightGroupOnly)
														{
															int num11 = _g2LIfRec[iPp][0].Item2.Stat_cnt();
															_g2LIfRec[iPp].RemoveAt(0);
															List<CcrClr> list9 = new List<CcrClr>();
															if (num11 > 0)
															{
																list9.Add(new CcrClr(_ccrHandler.GetCcrItem(_ccrHandler.GetName(ItemName.Ifmap, iPp))));
															}
															actionUpdater.UpdateG2LIf(segmentND2, ifmap, _lif, iPp, null, list9, 0, DataTypes.UInt8, ItemName.Ifmap, restored: false, 0, 0, 0, _h2C, _weightsShape[2], _strideH);
														}
														else
														{
													...
													bool flag3 = segment1D.End == _weightsShape[1] && item9.End == weight[3].End && item8.End == weight[2].End && i == num5 - 1 && num7 == num6 - 1;
													...
													if (!weightGroupOnly)
													{
														...
														int num13 = 0;
														if (flag3 && aCT0_OUTPUT_DEST == ACT0_OUTPUT_DEST.dm)
														{
															num13 = 1;
														}
														...
														if (num13 > 0)
														{
															list10.Add(new CcrSet(_ccrHandler.GetCcrItem(_ccrHandler.GetName(ItemName.Ofmap, ofPp)), 1));
														}
														int offsetL2GOf = 0;
														actionUpdater.UpdateR2LPsum(shift, segmentND, segmentND, psum, ofPp, l1Pp, aCT0_OUTPUT_DEST, releaseIf, Math.Max(i, num7), TcuComputeMode.NormalConv2d, loopStart, flag3, _strideH, _strideW, _ocPerGroup, _inputType, _weightType, ofType, _lact.CheckedDataType, list10, null, 0, offsetL2GOf, null, ItemName.Act, l2g_of_slice_from: false);
														if (flag3 && aCT0_OUTPUT_DEST == ACT0_OUTPUT_DEST.dm)
														{
															SegmentND segmentND4 = new SegmentND(segmentND[0], segmentND[2], segmentND[3], segmentND[1]);
															List<CcrClr> list11 = new List<CcrClr>();
															list11.Add(new CcrClr(_ccrHandler.GetCcrItem(_ccrHandler.GetName(ItemName.Ofmap, ofPp))));
															List<CcrSet> list12 = new List<CcrSet>();
															list12.Add(new CcrSet(_ccrHandler.GetCcrItem(_ccrHandler.GetName(ItemName.Ofmap2, ofPp)), 1));
															int offsetS = 0;
															int offsetD2 = 0;
															actionUpdater.UpdateMfuTranspose(segmentND, segmentND4, _outputType, MFU_TRANS_PERMUTE.NHWC, ofPp, list12, list11, offsetS, offsetD2, ItemName.Ofmap, ItemName.Ofmap2);
															List<CcrClr> list13 = new List<CcrClr>();
															list13.Add(new CcrClr(_ccrHandler.GetCcrItem(_ccrHandler.GetName(ItemName.Ofmap2, ofPp))));
															SegmentND ofmapStDdr = new SegmentND(segmentND4[2], segmentND4[3], segmentND4[0], segmentND4[1]);
															List<int> stridesD = new List<int>
															{
																ddrOf.FixedDimensions(2),
																ddrOf.FixedDimensions(0),
																ddrOf.FixedDimensions(1)
															};
															GNNEShape shape = new GNNEShape(segmentND4[0].Length, segmentND4[1].Length, segmentND4[2].Length, segmentND4[3].Length);
															actionUpdater.UpdateStoreT(segmentND4, _sof, ofPp, ddrOf, 0, ofmapStDdr, null, list13, ItemName.Ofmap2, null, stridesD, shape);
															ofPp = (ofPp + 1) % 2;
														}
													}
												}
						...
						if (!weightGroupOnly)
						{
							...
							SegmentND l2RIf = new SegmentND();
							List<CcrClr> list14 = new List<CcrClr>();
							if ((object)_act1 != null && _act1[GNNEActivation.InputB] != None.Default)
							{
								l2RIf = segmentND;
								int num14 = _l2RIf2Rec[ofPp][0].Item2.Stat_cnt();
								_l2RIf2Rec[ofPp].RemoveAt(0);
								for (int j = 0; j < num14; j++)
								{
									list14.Add(new CcrClr(_ccrHandler.GetCcrItem(_ccrHandler.GetName(ItemName.Ifmap2, ofPp))));
								}
							}
							if ((object)_dw != null)
							{
							...
							List<CcrSet> list15 = new List<CcrSet>();
							if (num15 > 0)
							{
								list15.Add(new CcrSet(_ccrHandler.GetCcrItem(_ccrHandler.GetName(ItemName.Ofmap, ofPp)), 1));
							}
							List<CcrClr> list16 = new List<CcrClr>();
							if (num16 > 0)
							{
								list16.Add(new CcrClr(_ccrHandler.GetCcrItem(_ccrHandler.GetName(ItemName.OfmapFake, ofPp))));
							}
							actionUpdater.UpdateL2RPsum(_dw, _pool, _act1, l2RIf, ifmap2, l2RDw, segmentND, l2GOf, psum, ofPp, l1Pp, ACT0_OUTPUT_DEST.dm, TcuComputeMode.NormalConv2d, _ocPerGroup, weightGroup, list14, list15, list16);
							l1Pp = (l1Pp + 1) % 2;
						}
						else
						{
	...
	private AllocateResult GetBoxes(int n, int c, int h, int w, int r, int s, int m, int e, int f, List<float> glbUsage)
	{
		...
		TensorOnGlb tensorOnGlb = new TensorOnGlb(new int[]{0,0,0,0}, DataTypes.UInt8, 0);
		int num = 0;
		if (_tp1)
		{
			int alignmentFactor = ((TileUtilities.GetBytesPerElement(_inputType) == 1) ? 32 : 16);
			int alignedNum = TileUtilities.GetAlignedNum(c, alignmentFactor);
			tensorOnGlb = new TensorOnGlb(new int[4] { n, h, w, alignedNum }, _inputType, 0);
			num = tensorOnGlb.AllocatedBytes * nPingPongSplit;
			list2[0] += num;
			num = TileUtilities.GetAlignedNum(num, GNNEEnv.IfmapBankWidth * GNNEEnv.GlbBankWidth);
			list2[1] += num;
		}
		int num2 = w;
		int alignmentFactor2 = ((TileUtilities.GetBytesPerElement(_inputType) == 1) ? 32 : 16);
		if (_tp1)
		{
			num2 = TileUtilities.GetAlignedNum(GNNEEnv.IfL1Size / GNNEEnv.PuHeight, alignmentFactor2);
		}
		TensorOnGlb tensorOnGlb2 = new TensorOnGlb(new int[4]
		{
			n,
			_tp1 ? GNNEEnv.PuHeight : c,
			h,
			num2
		}, _inputType, 0);
		int num3 = tensorOnGlb2.AllocatedBytes * nPingPongSplit;
		list2[0] += num3;
		num3 = TileUtilities.GetAlignedNum(num3, GNNEEnv.IfmapBankWidth * GNNEEnv.GlbBankWidth);
		list2[1] += num3;
		int num4 = c;
		if (num4 > _icPerGroup)
		{
		...
		TensorOnGlb tensorOnGlb3 = new TensorOnGlb(new int[4] { m, num4, r, s }, _weightType, 0);
		int num5 = SpaceSearcher.GetWeightSize(r, s, num4, m, TileUtilities.GetBytesPerElement(_weightType)) * GNNEEnv.NPingPongSplit;
		tensorOnGlb3.AllocatedBytes = num5 / GNNEEnv.NPingPongSplit;
		list2[0] += num5;
		num5 = TileUtilities.GetAlignedNum(num5, GNNEEnv.WBankWidth * GNNEEnv.GlbBankWidth);
		list2[1] += num5;
		int num6 = f;
		int alignmentFactor3 = ((TileUtilities.GetBytesPerElement(_outputType) == 1) ? 32 : 16);
		if (_tp2)
		{
			num6 = TileUtilities.GetAlignedNum(GNNEEnv.IfL1Size / GNNEEnv.PuHeight, alignmentFactor3);
		}
		TensorOnGlb tensorOnGlb4 = new TensorOnGlb(new int[4]
		{
			n,
			_tp2 ? GNNEEnv.PuWidth : m,
			e,
			num6
		}, _outputType, 0);
		int num7 = tensorOnGlb4.AllocatedBytes * GNNEEnv.NPingPongSplit;
		list2[0] += num7;
		num7 = TileUtilities.GetAlignedNum(num7, GNNEEnv.OfmapBankWidth * GNNEEnv.GlbBankWidth);
		list2[1] += num7;
		...
		int num8 = tensorOnGlb4.AllocatedBytes * GNNEEnv.NPingPongSplit;
		list2[0] += num8;
		num8 = TileUtilities.GetAlignedNum(num8, GNNEEnv.OfmapBankWidth * GNNEEnv.GlbBankWidth);
		list2[1] += num8;
		...
		list.Add(new BoxOnGlb(new int[2]
		{
			...
			num3 / GNNEEnv.IfmapBankWidth / GNNEEnv.GlbBankWidth
		}, ItemName.Ifmap));
		list.Add(new BoxOnGlb(new int[2]
		{
			GNNEEnv.WBankWidth,
			num5 / GNNEEnv.WBankWidth / GNNEEnv.GlbBankWidth
		}, ItemName.Weight));
		list.Add(new BoxOnGlb(new int[2]
		{
			GNNEEnv.OfmapBankWidth,
			num7 / GNNEEnv.OfmapBankWidth / GNNEEnv.GlbBankWidth
		}, ItemName.Ofmap));
		list.Add(new BoxOnGlb(new int[2]
		{
		...
		dictionary.Add(ItemName.Weight, tensorOnGlb3);
		dictionary.Add(ItemName.Ofmap, tensorOnGlb4);
		dictionary.Add(ItemName.Act, value2);
		if (_tp1)
		{
			list.Add(new BoxOnGlb(new int[2]
			{
				GNNEEnv.IfmapBankWidth,
				num / GNNEEnv.IfmapBankWidth / GNNEEnv.GlbBankWidth
			}, ItemName.Ifmap2));
			dictionary.Add(ItemName.Ifmap2, tensorOnGlb);
		}
		if (_tp2)
		{
			list.Add(new BoxOnGlb(new int[2]
			{
				GNNEEnv.OfmapBankWidth,
				num8 / GNNEEnv.OfmapBankWidth / GNNEEnv.GlbBankWidth
			}, ItemName.Ofmap2));
			dictionary.Add(ItemName.Ofmap2, value);
		}
		if (_weightType == DataTypes.UInt8 || _weightType == DataTypes.Int16)
		{
			int[] obj = new int[4] { 1, 1, 1, 0 };
			obj[3] = _convOutputShape[1];
			TensorOnGlb value3 = new TensorOnGlb(obj, _lwQarg.CheckedDataType, 0);
			...
			dictionary.Add(ItemName.WQarg, value3);
		}
		if ((object)_dw != null)
		{
			int fusedKernelH = _fusedKernelH;
			int fusedKernelW = _fusedKernelW;
			int num9 = _outputShape[1];
			DataType checkedDataType = _dw[GNNEPdp0DW.Weights].CheckedDataType;
			TensorOnGlb value4 = new TensorOnGlb(new int[4] { num9, 1, fusedKernelH, fusedKernelW }, checkedDataType, 0);
			int weightSize = SpaceSearcher.GetWeightSize(fusedKernelH, fusedKernelW, TileUtilities.GetAlignedNum(num9, GNNEEnv.PuWidth), 1, TileUtilities.GetBytesPerElement(checkedDataType));
			list2[0] += weightSize;
			...
			dictionary.Add(ItemName.DwWeight, value4);
			TensorOnGlb value5 = new TensorOnGlb(new int[4]
			{
				1,
				1,
				num9,
			...
			int actSize2 = SpaceSearcher.GetActSize(num9, GNNEEnv.ActNumPerChan, TileUtilities.GetBytesPerElement(_dw[GNNEPdp0DW.Act].CheckedDataType));
			list2[0] += actSize2;
			...
			dictionary.Add(ItemName.DwAct1, value5);
			if (checkedDataType == DataTypes.UInt8 || checkedDataType == DataTypes.Int16)
			{
				int[] obj2 = new int[4] { 1, 1, 1, 0 };
				obj2[3] = num9;
				TensorOnGlb value6 = new TensorOnGlb(obj2, checkedDataType, 0);
				int wQargSize2 = SpaceSearcher.GetWQargSize(num9, TileUtilities.GetBytesPerElement(checkedDataType));
				list2[0] += wQargSize2;
				...
				dictionary.Add(ItemName.DwQarg, value6);
			}
		}
		TensorOnGlb tensorOnGlb5 = new TensorOnGlb(new int[]{1,1,1,0}, DataTypes.Float16, 0);
		if ((object)_act1 != null && !_tp1)
		{
			bool num10 = ((TensorConst)_act1[GNNEActivation.Is16Segments]).Value.ToScalar<bool>();
			int num11 = (num10 ? 1 : _outputShape[1]);
			int num12 = (num10 ? 49 : GNNEEnv.ActNumPerChan);
			TensorOnGlb value7 = new TensorOnGlb(new int[4] { 1, 1, num11, num12 }, _act1[GNNEActivation.Act].CheckedDataType, 0);
			int actSize3 = SpaceSearcher.GetActSize(num11, num12, TileUtilities.GetBytesPerElement(_act1[GNNEActivation.Act].CheckedDataType));
			list2[0] += actSize3;
			...
			dictionary.Add(ItemName.MfuAct1, value7);
			if (_act1[GNNEActivation.InputB] != None.Default)
			{
				...
				int num13 = e;
				int num14 = f;
				int num15 = 32 / TileUtilities.GetBytesPerElement(dataType);
				if (e * f % num15 != 0)
				{
					int num16 = e + num15 - 1;
					int num17 = f + num15 - 1;
					long num18 = 4294836225L;
					for (int i = e; i < num16; i++)
					{
						for (int j = f; j < num17; j++)
						{
							int num19 = i * j;
							if (num19 % num15 == 0 && num19 < num18)
							{
								num18 = num19;
								num13 = i;
								num14 = j;
				...
				tensorOnGlb5 = new TensorOnGlb(new int[4] { n, m, num13, num14 }, dataType, 0);
				int num20 = tensorOnGlb5.AllocatedBytes * GNNEEnv.NPingPongSplit;
				list2[0] += num20;
				num20 = TileUtilities.GetAlignedNum(num20, GNNEEnv.IfmapBankWidth * GNNEEnv.GlbBankWidth);
				list2[1] += num20;
				list.Add(new BoxOnGlb(new int[2]
				{
					GNNEEnv.IfmapBankWidth,
					num20 / GNNEEnv.IfmapBankWidth / GNNEEnv.GlbBankWidth
				}, ItemName.Ifmap2));
			}
		}
		if (!_tp1)
		{
			dictionary.Add(ItemName.Ifmap2, tensorOnGlb5);
		}
		if ((object)_pool != null)
		{
			TensorOnGlb value8 = new TensorOnGlb(new int[4]
			{
				1,
				1,
				_outputShape[1],
			...
			dictionary.Add(ItemName.PdpAct1, value8);
		}
	...
	private void InitParameters(Call convNode, Call ld, Call st)
	{
		...
		_sof = st;
		_inputShape = new GNNEShape(1, _lif.CheckedShape[1].FixedValue, 1, _lif.CheckedShape[0].FixedValue);
		_outputShape = new GNNEShape(1, _conv.CheckedShape[1].FixedValue, 1, _conv.CheckedShape[0].FixedValue);
		...
		else if ((object)_act1 != null)
		{
			...
			TileUtilities.Assert(_outputShape.Dims.SequenceEqual(_convOutputShape.Dims), "_outputShape.Dims.SequenceEqual(_convOutputShape.Dims)", "/home/gitlab-runner/builds/zaC7hZ1H/0/maix2-ai-sw/k510-gnne-compiler/modules/Nncase.Modules.K230/Transform/Rules/Tile/TileTransposeMatmul.cs", 2144);
		}
		...
		_h2C = _weightsShape[1] * _weightsShape[2] <= GNNEEnv.PuHeight && _weightsShape[2] != 1 && _inputShape[2] > 200 && _inputShape[3] > 200 && !flag;
		if (_lif.CheckedShape[0].FixedValue > 1 && _conv.CheckedShape[2].FixedValue == 1 && _conv.CheckedShape[3].FixedValue == 1 && _lif.CheckedShape[2].FixedValue == 1 && _lif.CheckedShape[3].FixedValue == 1 && _weightsShape[2] == 1 && _weightsShape[3] == 1 && _strideH == 1 && _strideW == 1 && _dilationH == 1 && _dilationW == 1 && _paddingH.Sum() == 0 && _paddingW.Sum() == 0 && _groups == 1)
		{
			_tp1 = true;
			_tp2 = true;
		}
	...
	private void ArrangeWeights(DataType weightsType, GNNEShape weightsShape, Span<byte> oldWeights, WeightGroupHandler weightGroup)
	{
		...
		if (_h2C)
		{
			...
			{
				TileUtilities.Assert(num == weightGroup.WeightGroupOffset(item) * bytesPerElement, "offset == weightGroup.WeightGroupOffset(slice) * bytesPerElement", "/home/gitlab-runner/builds/zaC7hZ1H/0/maix2-ai-sw/k510-gnne-compiler/modules/Nncase.Modules.K230/Transform/Rules/Tile/TileTransposeMatmul.cs", 2399);
					...
					for (int j = 0; j < item[0].Length; j++)
					{
						for (int k = 0; k < item[3].Length; k++)
						{
							for (int l = 0; l < item[2].Length; l++)
							{
								...
								{
									int num3 = ((((j + item[1].Start) * weightsShape[1] + m + item[1].Start) * weightsShape[2] + l + item[2].Start) * weightsShape[3] + k + item[3].Start) * bytesPerElement;
		...
		else
		{
			...
			{
				TileUtilities.Assert(num4 == weightGroup.WeightGroupOffset(item2) * bytesPerElement, "offset == weightGroup.WeightGroupOffset(slice) * bytesPerElement", "/home/gitlab-runner/builds/zaC7hZ1H/0/maix2-ai-sw/k510-gnne-compiler/modules/Nncase.Modules.K230/Transform/Rules/Tile/TileTransposeMatmul.cs", 2426);
	...
	private void ArrangeDwWeights(DataType weightsType, GNNEShape weightsShape, Span<byte> oldWeights, WeightGroupHandler weightGroup)
	{
		int bytesPerElement = TileUtilities.GetBytesPerElement(weightsType);
		int num = 0;
		byte[] array = new byte[oldWeights.Length];
		foreach (SegmentND item in weightGroup.DwGroupSlice())
		{
			int start = item[1].Start;
			int end = item[1].End;
			int[] array2 = ((TensorConst)_dw[GNNEPdp0DW.Weights]).Value.ToArray<int>();
			SegmentND segmentND = new SegmentND(..1, start..end, ..array2[0], ..array2[1]);
			for (int i = 0; i < segmentND[0].Length; i++)
			{
				for (int j = 0; j < segmentND[2].Length; j++)
				{
					for (int k = 0; k < segmentND[3].Length; k++)
					{
						for (int l = 0; l < segmentND[1].Length; l++)
						{
							int num2 = ((((i + segmentND[0].Start) * weightsShape[1] + l + segmentND[1].Start) * weightsShape[2] + j + segmentND[2].Start) * weightsShape[3] + k + segmentND[3].Start) * bytesPerElement;
							for (int m = 0; m < bytesPerElement; m++)
							{
								array[num * GNNEEnv.PuWidth + l] = oldWeights[num2 + m];
							}
						}
						num++;
					}
