...
def build_onnx_and_kmodel(tag, W, outdir, want_probe=True):
    OUT, IN = W.shape
    ...
    calib = []
    for i in (1, 2):
        p = f"/tmp/kpu_poc/calib{S}/{tag}_{i}.npy"
        if os.path.exists(p):
...
def build_layer(li):
    ...
    for tag, W in gemm_specs(_R, li):
        ...
        entries.append(build_onnx_and_kmodel(f"l{li}_{tag}", W, OUTDIR, want_probe=(li == 0)))
    return entries
